...
function showResultPage(wpm) {
  resultPage.innerHTML = `<h2>Your WPM: ${wpm}</h2><button onclick="restartTest()">Try Again</button>`;
  resultPage.style.display = "block";  // Show the result page
  document.getElementById("test-container").style.display = "none";  // Hide the test container
}
// Initialize the word limit selection and generate initial text based on the default word limit
totalWordsToType();
wordLimit = text;  // Default to the 50-word array initially, but this will change on user selection
testGenerator();